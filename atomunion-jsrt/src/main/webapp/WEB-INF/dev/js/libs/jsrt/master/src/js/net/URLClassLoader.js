...
Class.forName({
    ...
    "protected loadClassInternal" : function(scriptUrl, synchronous, notModify, callback, $scope, showBusy) {
        ...
        var loadedScripts = this.loadedScripts, waitingList = this.waitingList, completed = [], failed = [], scope = this;
        ...
        var synchronousScript = function(url, src) {
            var isCrossOriginRestricted = false, xhr, status, isIE = /msie/.test(navigator.userAgent.toLowerCase()), debugSourceURL = isIE ? "" : ("\n//@ sourceURL=" + src);
...
$import = function(name, classloader) {
